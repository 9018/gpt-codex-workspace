...
test("create_goal accepts implementation as a product alias and creates a full worktree task", async () => {
  const server = await makeServer();
  const created = await callTool(server, "create_goal", {
  ...
  assert.equal(created.goal.acceptance_contract.intent.execution_mode, "full");
