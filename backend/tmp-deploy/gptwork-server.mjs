...
export async function createGptWorkServer(options = {}) {
  ...
  return {
    ...
    async listen({ host = "127.0.0.1", port = 8787 } = {}) {
      const server = http.createServer((req, res) => handleHttp(req, res, this));
...
export function startCodexWorker(server, {
  ...
  async function tick() {
    ...
    running = true;
    try {
      await server.runAssignedCodexTasks({ limit, concurrency });
    } catch (error) {
      process.stderr.write(`[gptwork-worker] ${error.message}\n`);
    } finally {
      running = false;
...
function createTools({ store, config, browser, github }) {
  ...
  return {
    ...
    delete_workspace: tool("Delete a workspace registration without deleting remote files.", schema({ workspace_id: "string" }, ["workspace_id"]), async (args, context) => deleteWorkspace(store, args, context)),
    ...
    run_assigned_codex_tasks: tool("Run safe built-in Codex task handlers for currently assigned tasks. Supports bounded concurrent execution. This only executes approved internal handlers such as Codex session metadata inventory; it does not run arbitrary task descriptions or shell commands.", schema({ limit: "integer", concurrency: "integer" }), async (args, context) => runAssignedCodexTasks(store, config, github, args, context)),
    ...
    delete_path: tool("Permanently delete a file or directory.", schema({ path: "string", recursive: "boolean", workspace_id: "string" }, ["path"]), async (args, context) => workspaceDelete(store, config, args, context)),
    ...
    shell_exec: tool("Execute a shell command in a workspace.", schema({ command: "string", cwd: "string", timeout: "integer", max_output_bytes: "integer", workspace_id: "string" }, ["command"]), async (args, context) => workspaceShellExec(store, config, args, context)),
...
async function runAssignedCodexTasks(store, config, github, { limit = 10, concurrency = 4 } = {}, context = defaultTokenContext("system")) {
  ...
  const candidates = state.tasks
    .filter((task) => task.assignee === "codex" && task.status === "assigned" && canAccessProject(context, task.project_id) && canAccessWorkspace(context, task.workspace_id))
    .slice(0, maxTasks);
