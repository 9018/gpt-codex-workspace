...
export function renderGoalMarkdown(goal, conversation, memories, task, workspaceFiles) {
  ...
  lines.push(
    ...
    "## Autonomy Policy",
    "",
    `Mode: ${goal.autonomy_policy?.mode || 'single_runner'}`,
    ...
    "Resolve uncertainty directly when practical; use subagents only when they materially improve complex work.",
    "",
    "## Subagent Policy",
    "",
    `Mode: ${goal.subagent_policy?.mode || "disabled"}`,
    "Required roles:",
    ...(() => {
      const roles = goal.subagent_policy?.canonical_roles || goal.subagent_policy?.roles || [];
      return Array.isArray(roles) && roles.length > 0 ? roles.map((role) => `- ${role}`) : ["(none)"];
    })(),
    "",
    "## Execution Contract",
    "",
...
export function codexInstruction(goal) {
  ...
  return [
    ...
    "",
    "You are the parent Codex executor responsible for this goal.",
    "For simple work, execute directly. For complex work, you may start subagents, but do not require a fixed role pipeline.",
    "",
    "Before asking ChatGPT:",
    "1. Analyze the issue directly or delegate focused analysis to subagents when materially useful.",
    "2. Compare options.",
