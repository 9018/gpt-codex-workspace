...
export function createWorkspaceReadToolsGroup({ tool, schema, store, config }) {
  return {
    ...
    download_bundle_base64: tool("Create a ZIP bundle from a workspace directory or selected paths and return it as base64 with a SHA256 digest.", schema({ source_dir: "string", paths: "array", max_bytes: "integer", workspace_id: "string" }, []), async (args, context) => workspaceDownloadBundleBase64(store, config, args, context)),
    search_files: tool("Search text content and file names under a directory.", schema({ q: "string", path: "string", limit: "integer", exclude_dirs: "array", max_file_bytes: "integer", workspace_id: "string" }, ["q"]), async (args, context) => workspaceSearch(store, config, args, context)),
