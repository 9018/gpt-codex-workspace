...
try { await reconcilePendingRestartMarkers(config.defaultWorkspaceRoot, config.defaultRepoPath); } catch (e) { warnings.push({ code: "restart_marker_reconcile_failed", message: e?.message || String(e) }); }
        ...
        const result = {
          ...
          agent_local_command_configured: Boolean(config.agentLocalCommand),
          agent_role_commands: Object.fromEntries(Object.entries(config.agentRoleCommands || {}).map(([role, command]) => [role, Boolean(command)])),
          agent_command_timeout: config.agentCommandTimeout,
          ...
          config_sources: {
            ...
            agent_backend: sources.agentBackend,
            agent_role_backends: sources.agentRoleBackends,
            agent_local_command: sources.agentLocalCommand,
            agent_role_commands: sources.agentRoleCommands,
            agent_command_timeout: sources.agentCommandTimeout,
            ...
            render_mode: sources.renderMode,
