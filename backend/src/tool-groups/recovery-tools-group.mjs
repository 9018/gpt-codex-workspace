...
const ALLOWLISTED_COMMANDS = {
  ...
  runtime_status: { cmd: (rp) => `cd ${rp}/backend && curl -s http://localhost:${process.env.GPTWORK_PORT||8787}/mcp/health 2>/dev/null || echo "MCP not responding"`, desc: "Runtime health check" },
