...
import { classifyClosure } from "../auto-closure-classifier.mjs";
...
import { continueOnCompletedOutcome, goalStatusFromReconciliation } from "../closure/continuation-flow.mjs";
import { runAcceptanceGate } from "../acceptance-gate-engine.mjs";
import { applyTaskFinalStateDecision, decideTaskFinalization } from "./task-final-state-decider.mjs";
...
import { writeDefaultFinalizationAgentRuns } from "./finalization-notifier.mjs";
import { collectTaskFinalizerEvidence } from "./task-finalization-facts.mjs";
import { projectGoalStatusForFinalizedTask } from "./goal-state-projection.mjs";
import { runFinalizationStateTransition, runPostFinalizationEffects } from "./task-finalization-effects.mjs";
import {
...
} from "./repair-finalizer.mjs";
import { assertValidInputUnifiedDecision } from "./finalization-errors.mjs";
import {
...
} from "./task-finalizer-orchestrator.mjs";
import {
...
} from "./finalization-proofs.mjs";
import { releaseFinalizationRepoLock } from "./worktree-cleanup.mjs";
...
export async function finalizeCodexTaskRun({
  ...
  if (runFilePath) {
    const resolvedRjPath = resultJsonPath || (workspace.root + "/.gptwork/goals/" + (goal ? goal.id : task.id) + "/result.json");
    fireHeartbeatFn(runFilePath, taskStatus === "completed" ? "completed" : "failed", {
      result_json_path: resolvedRjPath,
