...
function mapZvecResult(r, scoreKind = "cosine_similarity") {
  ...
  return {
    ...
    metadata: {
      ...
      root_goal_id: fields?.root_goal_id || "",
      parent_goal_id: fields?.parent_goal_id || "",
      phase: fields?.phase || "",
      iteration: Number(fields?.iteration || 0),
      shard_key: fields?.shard_key || "",
      lineage_depth: Number(fields?.lineage_depth || 0),
  ...
  try {
    ...
    const schema = new ZVecCollectionSchema({
      ...
      fields: [
        ...
        { name: "workstream_id", dataType: ZVecDataType.STRING, nullable: true },
        { name: "root_goal_id", dataType: ZVecDataType.STRING, nullable: true },
        { name: "parent_goal_id", dataType: ZVecDataType.STRING, nullable: true },
        { name: "phase", dataType: ZVecDataType.STRING, nullable: true },
        { name: "iteration", dataType: ZVecDataType.INT64 },
        { name: "shard_key", dataType: ZVecDataType.STRING, nullable: true },
        { name: "lineage_depth", dataType: ZVecDataType.INT64 },
        { name: "freshness", dataType: ZVecDataType.STRING, nullable: true },
      ...
      async addChunks(chunks, vectors, options = {}) {
        ...
        const docs = chunks.map((chunk, i) => ({
          ...
          fields: {
            ...
            workstream_id: String(metadataValue(chunk.metadata, "workstream_id", "")),
            root_goal_id: String(metadataValue(chunk.metadata, "root_goal_id", "")),
            parent_goal_id: String(metadataValue(chunk.metadata, "parent_goal_id", "")),
            phase: String(metadataValue(chunk.metadata, "phase", "")),
            iteration: Number(metadataValue(chunk.metadata, "iteration", 0)),
            shard_key: String(metadataValue(chunk.metadata, "shard_key", "")),
            lineage_depth: Number(metadataValue(chunk.metadata, "lineage_depth", 0)),
            freshness: String(metadataValue(chunk.metadata, "freshness", "")),
