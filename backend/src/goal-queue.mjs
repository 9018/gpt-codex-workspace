  ...
  if (item.depends_on_goal_id || item.depends_on_task_id) {
    ...
    if (!depState.effective_completed) {
      ...
      if (prerequisiteTask && prerequisiteTask.status === 'completed') {
        const prResult = prerequisiteTask.result || {};
        // AFC-P3 + follow-up convergence: use unified_decision as the primary signal,
        // then fall back to finalizer/closure decisions for historical task results.
        const prUD = prResult.unified_decision || {};
        if (prUD.queue_effect?.unblock_dependents === true || prUD.safe_to_auto_advance === true) {
          // Canonical signal says dependents may advance.
